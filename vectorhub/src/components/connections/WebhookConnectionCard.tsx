...
export function WebhookConnectionCard({ connection, onDelete, onEdit }: WebhookConnectionCardProps) {
            ...
            <CardContent>
                ...
                {connection.eventTypes.length > 0 && (
                    <p className="mt-2 text-xs text-muted-foreground truncate">
                        Events: {connection.eventTypes.join(", ")}
                    </p>
                )}
                <p className="text-xs text-muted-foreground mt-2">
                    Last delivery{" "}
                    {connection.lastDelivery
                        ? formatDistanceToNow(new Date(connection.lastDelivery), {
                              addSuffix: true,
                          })
                        : "not yet"}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                    Secret: {connection.secretConfigured ? "configured" : "not configured"}
                </p>
            </CardContent>
            {(onDelete || onEdit) && (
